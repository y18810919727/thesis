...
另外，在训练阶段，为了使该模型能够支持批量并行化推理与训练，需要解决同一batch中求解时间点彼此不同、积分区间彼此不一致的问题。
...
在实验环节，我们在共3个系统辨识数据集(2个公共数据集和1个私有数据集)上评估了所述模型在长期开环预测问题中的性能。实验表明在非均匀采样下的随机系统预测任务上，ODE-RSSM无论是在处理非均匀采样方面还是建模系统随机性方面均具有良好表现，预测精度显著优于离散时间状态空间模型以及带有确定性状态演变的连续时间模型。
...
% Dynamics model learning, also known as System identification~\cite{aastrom1971system}, is a task that learns the dynamical model of a inputs-outputs system from offline data.
% It raises an important role in areas of system prediction and model-based Reinforcement learning. 
动态系统建模，也被称为系统辨识~\cite{aastrom1971system}，是一项从离线数据中学习输入输出系统动态模型的任务。
其广泛应用于系统仿真预测及基于模型的强化学习领域。
% The learned model can be used in control, state estimation, and prediction.
% In this paper, we focus on the black-box system modelling where both of the functional forms and parameters are unknown.
% In order to model the unknown non-linear complicated systems, approximate parametric model based on neural networks are able to flexibly approximate the systems with non-linear functions and high dimensional inputs\cite{wang2017new,ogunmolu2016nonlinear}.
在本章中，我们重点关注函数结构及系统参数均未知的黑盒系统辨识任务。
为了对此类未知非线性复杂系统进行建模辨识，基于神经网络的参数近似模型能够灵活地对具有非线性动态和高维输入的系统进行拟合近似~\cite{wang2017new,ogunmolu2016nonlinear}。
% In many real-world systems, the sequential data are sampled unevenly from the identified system. 
% SNODE\cite{Quaglino2019} uses the spectral element methods for fast and accurate training of Neural Ordinary Differential Equations for system identification. 
% ODE-RNN\cite{Rubanova2019} inserts an ODE-Net module between adjacent RNN updates for modelling the continuous-time evolution of hidden states.
% In order to improve the long-term prediction, previous studies \cite{Demeester2019,Yuan2022} incorporate the advantages of recurrent network and differential equation to tackle the unit root problem and propose the Time-Aware RNN, as a continuous-time system identification model.

在许多真实的工业场景中，从被识别系统采样得到的序列数据经常是间隔非均匀的。
...
% In order to improve the stability of long-term prediction, Time-Aware RNN~\cite{Srinivas2020,Yuan2022} incorporate the advantages of recurrent network and differential equation to tackle the unit root problem.
% and propose the Time-Aware RNN, as a continuous-time system identification model.
% tackle the unit root problem and utilize a time-aware and stationary RNN for long-term prediction.
% The models above are conti
% The above-mentioned continuous-time models are capable of tackling the problem of unevenly sampling, but are weak in modeling stochastic systems because their internal states evolution is deterministic.
上述连续时间模型能够有效解决系统的不均匀采样问题，但由于其模型内部状态的转换是确定性的，因此对于存在随机性的系统，其建模能力较弱。
% 但由于随机系统内部状态演化具有确定性，因此对随机系统建模能力较弱。

% In the perspective of modelling stochastic system, deep temporal generative models~\cite{Fraccaro2016,Chung2015,Karl2017} extend the Variational Auto-encoders (VAEs)~\cite{kingma2013auto} to sequential situations.
...
% Introducing stochastic latent variables enables the models to learn the stochasticity in the sequences
% The models are capable of learn the stochasticity in the sequences by introducing stochastic latent variables.
% PR-SSM~\cite{doerr2018probabilistic} and PILCO~\cite{deisenroth2011pilco} introduce Gaussian Processes to learn the probabilistic state space models of input/output systems.
此类模型通过引入时序的随机隐变量学习序列的随机性。
类似地，PR-SSM~\cite{doerr2018probabilistic}和PILCO~\cite{deisenroth2011pilco}采用高斯过程来学习输入/输出系统的概率状态空间模型。
% , which is realized by learning
% They learn the representations of the sequential data and generate new instances from the same distributions or conditional distributions.
% With the benefits of efficient sampling and prediction, the learned generative models are employed in many applications, such as missing data imputation\cite{Fraccaro2017}, open-loop prediction\cite{Hafner2019} and model-based reinforcement learning(RL)\cite{Hafner2019}.
...
% With the approximate posterior model for inferring latent variables, high-dimensional temporal observations can be compressed to low-dimensional state representations, which is widely used in visual encoding\cite{Li2018a} and image-based control\cite{Lee2019}.
% The generative model is capable of efficient sampling and prediction, which has been used in
% These model with both functions been used in many applications, such as missing data imputation\cite{Fraccaro2017}, open-loop prediction\cite{Hafner2019} and model-based reinforcement learning(RL)\cite{Hafner2019}.

% To leverage the advantages of stochastic model to quantify the uncertainty of identified system, 
% Some previous works also integrate the temporal variational inference framework and differential networks to learn a stochastic continuous-time process. 
以往的一些工作尝试将时序变分推理框架与微分方程网络相结合，用于学习随机的连续时间过程处理非均匀采样的序列数据。
% LatentODE\cite{Rubanova2019}, ODE2VAE\cite{Yildiz2019} and LatentSDE\cite{Li2020} infer the posterior distribution of latent state from the irregular observed sequences. 
...
% By solving the differential network with an initial state which is sampled from the inferred posterior distribution, the model support interpolating the internal missing positions and extrapolating the system outputs in future. 
% With an sampled initial state from the inferred posterior distribution, model can interpolate the missing positions and predict the system outputs by solving the differential network.
% While these work are offline models, as 
% However, these models are offline, because they have to read the complete irregular time series for encoding before prediction and interpolation\cite{Liu2020}.
然而，由于这些模型在预测或插值前必须输入完整的采样序列以用于隐状态的推理，因此这些模型均是离线模型\cite{Liu2020}。
在某些特定任务中，如在线控制，预测模型需要能够运行在在线模式下以处理流式数据。如循环神经网络，可以支持递归地输入单点数据并更新其内部状态。
% In actual industrial intelligent system, such as MPC, the prediction model is required to be working in online mode and recurrent manner for reducing the computing load.
% In some specific tasks, such as online control, the prediction model is required to be working in online mode for handling streaming data\cite{lesort2018state}, just like the recurrent neural network which updates internal state recursively.

% In backward propagation, the gradient flow in $p_{\theta}\left(\boldsymbol{z}_{t} \mid \boldsymbol{h}_{t}\right)$ depends on the KL-divergence in single step $t$.
% Generally, there is long-time delay in the influences from system inputs on system outputs in most complicated industrial system.
% It is a critical issue to guarantee the long-term forecasting of the prediction.
% However, the loss function in \eqref{equ:elbo_single} are unfavorable for guaranteeing the accuracy of long-term prediction.

% One or parts of issues mentioned above are indeed solved by previous studies.
% In this paper, we propose a novel ODE-RSSM model which satisfies the all requirements in actual industrial applications, including of learning irregular sampled data, modelling stochasticity, and online prediction.
% The comparison of the proposed model and other neural-based system modelling methods is shown in \ref{tab:comparison}.

本章针对上述问题，提出了一种新颖的ODE-RSSM模型。该模型能够满足实际工业应用场景下的对于预测模型的上述所有要求。包括利用不均匀采样数据进行学习、建模动态系统的随机性以及支持在线预测。
相比于其他基于神经网络的系统建模方法，本章提出模型的优势如表\ref{tab:comparison}所示。
...
% Taking the Recurrent State Space Model(RSSM)~\cite{Hafner2019} for example, the evolved state includes a deterministic path $\b{h}_{i}$ and a stochastic path $\b{z}_{i}$.
以循环状态空间模型(Recurrent State Space, Model, RSSM)~\cite{Hafner2019}为例，模型的内部的更新状态同时包括确定性变量$\b{h}_{i}$和随机变量$\b{z}_{i}$。
利用生成模型参数$\theta$，定义离散时间域下序列隐变量和观测值的联合概率如下:
% The joint probability of the sequential latent states and observations in discrete-time domain is defined with generative parameters $\theta$: 
% \begin{equation}
% \begin{aligned}
% p_{\theta}\left(\b{y}_{1: N}, \b{z}_{1: N},\right.&\left.\b{h}_{1: N} \mid \b{u}_{1: N}, \b{h}_{0}\right)=p_{\theta}\left(\b{y}_{1: N} \mid \b{z}_{1: N}, \b{h}_{1: N}\right) \times \\
% & \times p_{\theta}\left(\b{z}_{1: N} \mid \b{h}_{1: N}\right) \widetilde{p}\left(\b{h}_{1: N} \mid \b{u}_{1: N},\boldsymbol{z}_{1: N}, \b{h}_{0}\right),
% \end{aligned}
% \end{equation}
\begin{equation}
\begin{aligned}
...
确定性转移函数$\widetilde{p}_{\theta}(\b h_{i}|\cdot)$为一delta分布，由一个确定性的循环神经网络进行建模。
随机隐状态$p_{theta}\left(\b {z}_{t} \mid \b {h}_{t}\right)$和解码网络$p_{theta}\left(\b {y}_{i} \mid \bold {h}_{i},\b {z}_{i}\right)$的条件分布均为参数化的高斯分布。
...
为了训练生成参数$\theta$， 一般地，通过引入近似后验编码器 $q_{\phi}\left(\boldsymbol{z}_{i} \mid \boldsymbol{y}_{i}, \boldsymbol{h}_{i}\right)$对隐变量的真实后验分布进行近似，以最大化证据下限(Evidence Lower Bound, ELBO)的方式协同训练参数 $\phi$ 和$\theta$：
\begin{equation}
\begin{aligned}
...
在生成模型中，在给定系统输入的情况下，模型通过组合GRU网络和ODE网络建模隐状态的转移过程。
% In the generative process, the joint probability of latent states and system outputs with the given system inputs is modeled by the combination of a GRU network and an ODE-Net.
% The distribution of system outputs is also predicted 
% by the condition of system inputs.
在推理过程中，模型引入了编码器模型用于在已知系统输出和输入序列的情况下，推断隐状态的近似后验分布。
% Furthermore, we introduce an efficient latent overshooting technique to generalize the standard single-step variational bound to multi-step setting, which improves the multi-step predictions in latent space.
% On the other hand, we further introduce the latent overshooting\cite{Hafner2019} technique in training phase to improve the long-term prediction.
% For enabling the technique to be efficient in training ODE-RSSM, we improve the efficiency of state sampling process by reusing the temporary sampled states and extending the solving of batched ODE-Net to be parallelizable in irregular sampling setting
另一方面，我们在训练阶段进一步引入隐空间超调技术\cite{Hafner2019}用于改善模型的多步预测。
为了减少引入隐空间超调带来的额外时间开销，进而优化训练效率。本章通过重用临时采样状态的方式显著减少了求解多步KL散度的时间复杂度。
% 为了使该技术能够有效地训练ODE-RSSM，我们通过重用临时采样状态和将批处理ODE-Net的求解扩展到不规则采样设置下的并行化来提高状态采样过程的效率
% optimizing the time complexity and extend the batched ODE solving to be parallelizable in irregular sampling setting.
...
对于任一输入输出系统，
对其非均匀采样数据进行定义，包括序列系统输入为$\{\ut\}_{i=1}^{N}$，以及序列系统输出为$\{\yt\}_{i=1}^{N}$，其中$t_i$为$i$-th位置数据点的采样时间，$N$为完整序列长度。
...
与RSSM\cite{Hafner2019}模型相似，ODE-RSSM中的隐状态由确定性部分$\hti$和随机部分$\zt$共同组成。
...
ODE-RSSM模型的目标是学习深度条件序列生成模型，该模型的推断模块能够在给定系统输入和输出的情况下推断序列隐状态$\{\st\}_{i=1}^{N}$，其生成模型能够在给定系统输入$\{\ut\}_{i=1}^{N}$下，预测系统输出$\{\yt\}_{i=1}^{N}$的条件分布.
...
在式.~\eqref{equ:prob_latent}中， $p\left(\boldsymbol{h}_{t_{i}} \mid \boldsymbol{h}_{t_{i-1}}, \boldsymbol{u}_{t_{i-1}}\right)$为狄拉克分布，并且在给定$t_{i-1}$时刻的的隐变量$\st$和系统输入$\ut$下，$\hti$是确定性的。
...
\subsection{非均匀采样间隔下的批量常微分方程并行化求解方法}
...
对于任一标量函数$f(\cdot)$，其在区间$[a, b]$上的定积分满足：
\begin{equation}
...
% \newtheorem{proposition}{\indent Proposition}
% \begin{proposition}
% \textbf{Interval Transformation for Ordinary Differential Equation:}
...
近似后验分布和预测先验分布之间的差异打破了生成模型输入在训练集和测试集上的同分布假设，这可能导致多步预测的准确性较差\cite{venkatraman2015改进}。
...
根据文献~\cite{Hafner2019}给出的证明, $\ln p_d\left(\boldsymbol{y}_{t_{1}: t_{N}}\right)$ 是式\eqref{equ:elbo_single}给出的单步预测ELBO的下限，自然也是原始系统输出对数似然的下限。
...
% The complete algorithm \textbf{Latent overshooting based on parallel ancestor sampling} is shown in Alg.\ref{alg:parallel_overshooting}.
...
% \begin{algorithmic}[1]
% \Statex \textbf{Input: } The generative networks (an ODE-net $f_\theta$ and a GRU), the encoder for posterior inferring, sequential inputs $\b u_{t_1:t_N}$ and sequential outputs $\b y_{t_1:t_N}$
% % \Statex \textbf{Output:} $\text{KL-D}(q_{\phi}(\b z_{t_1:t_N})||p_{\theta}(\b z_{t_1:t_N}|\b u_{t_1:t_N}))$ 
% \Statex \textbf{Output:} $\text{KL-D}$ 
...
\hline
\textit{CSTR}       & 1      & 2       & +          & +          & 60  & 180  \\
\textit{Winding}    & 5      & 2       & +++        & ++         & 60  & 180  \\
\textit{Thickening} & 4      & 1       & ++         & +++        & 30  & 60   \\
\bottomrule
\end{tabular}
}
\caption{数据集特性}
...
% \textbf{Narendra-Li Benchmark from} 
...
% \textbf{Paste Thickening Pressure Dataset} 
...
生成的每一批数据均为大小为$B \ * N\ * K$的张量，其中$B$是批大小，$N$表示序列的长度，$K=\text{dim}_{in}+\text{dim}_{out}$是系统输入和系统输出的维数之和。
...
% ODE-RNN               & 0.5018 & 0.4953                   & \textbf{0.402}  & \textbf{0.4079}                 & \textbf{0.3947} & \textbf{0.3908}                   & 0.3852 & 0.3891                 & 0.3808 & 0.3751            \\
...
提出的ODE-RSSM与离散时间模型在三个数据集上的比较如表所示。\ref{制表符:cstr}， \ref{制表符:winding}，和\ref{制表符:thickening}。
...
从表\ref{tab:}可以发现，训练阶段添加隐空间超调能够有效改善模型的开环预测精度，且由于浓密机数据集存在更明显的系统时延于非确定性，改善程度更加明显。
...
\toprule
\multirow{2}{*}{Model} & \multicolumn{4}{c|}{25\%(uneven)}                            & \multicolumn{4}{c|}{50\%(uneven)}                            & \multicolumn{4}{c}{100\%}                            \\
...
\caption{
The results of evaluating ODE-RSSM and RSSM when the sampling ratio is different from the ratio in training dataset.
}
...
\begin{figure}[ht]
\subfigure[Reusing sampled latent states]{
\begin{minipage}[t]{0.45\linewidth}
\centering
% \hspace{-22pt}
% \includegraphics[width=1\linewidth,trim=0 0 0 0,clip]{figures/chapter5/D_time.eps}
\includegraphics[height=4.5cm]{figures/chapter5/D_time.eps}
%\caption{fig1}
\end{minipage}
\label{fig:D_time}
}%
\subfigure[Parallel ODE solver]{
\begin{minipage}[t]{0.45\linewidth}
\centering
% \hspace{-22pt}
% \includegraphics[width=1\linewidth,trim=0 0 0 0,clip]{figures/chapter5/ode_time.eps}
\includegraphics[height=4.5cm]{figures/chapter5/ode_time.eps}
%\caption{fig1}
\end{minipage}
\label{fig:ode_time}
}%
\centering
\caption{Evaluating the time consumption}
\label{fig:time_cmp}
\end{figure}
...
本节将围绕ODE-RSSM具备的在线编码、在线预测的能力，探讨该模型在系统预测以外的潜在应用。
% On the basis of such property, this section will discuss the available applications of ODE-RSSM except for system prediction.
 ...
 \textbf{模型预测控制} 
在模型预测控制中(Model Prediction Control, MPC)，ODE-RSSM可以作为优化过程所需的预测模型或仿真模型，
...
% \textbf{Asynchronous observations} 
...
\textbf{时间演化过程中的随机性建模} 
% ODE-RSSM is intrinsically a discrete-time model, which only infers the stochastic latent variables at discrete-time steps. 
