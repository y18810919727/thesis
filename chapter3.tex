...
对于上述复杂工业系统特点，许多工业界和学术界的研究人员通过构建不同的预测模型来解决这些问题。
...
然而，非平稳形式\eqref{equ:3_non_stationary}在处理长期预测任务时面临着严重的问题。
...
%\vspace*{-0.3cm}
\end{figure}
图中呈现的预测效果说明稳定型的导数模块在长序列预测时的表现更优，与表\ref{tab:3_exp_all}中的结果几乎一致。而非平稳模型在长期预测任务中的RRSE和MSE远远高于平稳模型，模型预测效果较差。
