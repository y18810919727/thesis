...
对于膏体制备系统数据集，本节从DCS系统中采集约11万条数据用于模型的训练与评价，所有数据均来源于生产运行稳定时段，且不同时段浓密机出料流量和浓度和有较大差异，流量波动范围为$130m^3/h-200m^3/h$，浓度波动范围为$52\%-67\%$。
...
本节同时引入单ODE-Net、ODE-RNN\cite{10.5555/3454287.3454765}、CDE-Net\cite{kidger2020neural}作为本章AJ-ODE-Net的对比模型。除单ODE-Net外，对比模型也可以与本章的阶段转换预测器组合，进而具备跳变系统辨识能力。AJ-ODE-Net与其他模型的对比结果如图\ref{fig:4_paste_models}所示。
...
\caption{不同模型预测水泥添加量、膏体流量、膏体浓度的效果对比。\textcolor{orange}{橙色}：开机阶段，\textcolor{blue}{蓝色}：关机阶段} %图片标题
...
\begin{figure*}
    ...
    \caption{不同模型预测进气口温度、制冷量、制冷机功率的效果对比。\textcolor{orange}{橙色}：制冷系统启动第一阶段，\textcolor{green}{绿色}：制冷系统启动第二阶段，\textcolor{blue}{蓝色}：制冷系统关闭，\textcolor{red}{红色}：稳定制冷阶段} %图片标题
...
图~\ref{fig:paste_bound_simulation}展示了设置不同浓度上限时，膏体制备系统在2小时内的水泥添加量变化情况。
% todo: 上面的1000s
% 对制冷系统的影响。
% 仿真时间为1000秒。
