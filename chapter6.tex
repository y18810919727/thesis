...
\section{本章方法在工业浓密机仿真系统底流浓度控制中的应用}
...
\footnotetext[1]{平均平方误差(Mean Square
 Error, MSE)=$\frac{1}{T} \sum_{k=1}^{T}\left|(y(k)-y^*(k))\right|^{2}$} \footnotetext[2]{最大绝对误差(Max Absolute
 Error, MAE)=$\max _{1 \leq k \leq T}\{|y(k)-y^*(k)|\}$} \footnotetext[3]{累积绝对误差(Integral Absolute
...
具体地，我们分别采用HCNVI算法以及系统自带规则控制算法对浓密机底流浓度进行控制。每种算法下进行三组实验，每次实验过程持续1小时，对于每组结果，我们统计了五个指标量以评估不同算法的追踪效果以及浓度稳定效果，三组实验的实验结果以及平均值、最大值如\tabref{tab:industrial_thickening_eval}所示：
\begin{table}[htb]
  \centering
