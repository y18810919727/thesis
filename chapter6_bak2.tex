...
本节基于系统先验知识构建了\tabref{tab:paste_dfa}所示的系统阶段转换的判断规则，并基于此自动地为序列数据赋予阶段标签。
...
\caption{不同模型预测水泥添加量、膏体流量、膏体浓度的效果对比。\textcolor{orange}{橙色}：On阶段，\textcolor{blue}{蓝色}：Off阶段} %图片标题
...
\end{equation}
绝对百分比误差$\text{APE}(t_1,t_2)$衡量了时间范围$[t_1, t_2]$内水泥消耗预测值和真实消耗之间的相对误差。
